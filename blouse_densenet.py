...
parser.add_argument('--batchsize',default = 16, type = int, help = 'batch aize')
parser.add_argument('--max_iter',default = 60, type=int,help='max iteration of training set')
...
resnet18.classifier = torch.nn.Sequential(torch.nn.SELU(inplace = True),torch.nn.Linear(1024,26)) #1024-121 1664-169
#resnet18.load_state_dict(torch.load('checkpoints/c-0.02-8-densenet121-30.pth'))
if use_cuda:
...
#optimizer = torch.optim.SGD(resnet18.parameters(),lr = args.lr ,momentum = 0.90)
optimizer = torch.optim.Adam(resnet18.parameters(), lr=args.lr, betas=(0.5, 0.9), eps=1e-08, weight_decay=5e-4)
...
for epoch in range(args.max_iter):
    ...
    if epoch % 2 == 0:
         save_model(resnet18,'checkpoints/pre-0.02-8-densenet121-{}.pth'.format(epoch))
...
save_model(resnet18,'checkpoints/pre-0.02-8-densenet121-{}.pth'.format(args.max_iter-1))
print('finish......')
